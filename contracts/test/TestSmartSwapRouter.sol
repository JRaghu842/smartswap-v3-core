...
contract TestSmartSwapRouter is ISmartSwapSwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        ISmartSwapPool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function smartSwapSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? ISmartSwapPool(msg.sender).token0()
                : ISmartSwapPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0
                ? amount0Delta
                : amount1Delta;

            bool zeroForOne = tokenToBePaid ==
                ISmartSwapPool(pools[0]).token1();
            ISmartSwapPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
                zeroForOne
